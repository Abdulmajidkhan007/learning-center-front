...
export function TeacherDashboardPage() {
  ...
  const selectedGroupId = visibleGroups.some(
    (group) => group.id === pickedGroupId,
  )
    ? pickedGroupId
    : (visibleGroups[0]?.id ?? "");
  ...
  const students = useMemo(
    () => groupInfoQuery.data?.studentDto ?? [],
    [groupInfoQuery.data],
  );
  ...
  function openAttendance() {
    ...
    navigate("/attendance", {
      state: { activeLesson, groupId: selectedGroupId },
    });
  }
  ...
  return (
    ...
    >
      ...
      {/* Tor ekranda tasma, kengida chap ustun — administrator va
                super-admin panellaridagi bilan bir xil tuzilma. */}
      <GroupTabs
      ...
      <div className="flex gap-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          {loadError && (
            <div className="mb-5">
              <ErrorBox>
                {t("teacher.loadFailed", { message: errorMessage(loadError) })}
              </ErrorBox>
            </div>
          )}
          ...
          {!isLoading && !loadError && !group && (
            <EmptyState
              title={
                allGroups.length === 0
                  ? t("teacher.noGroups")
                  : t("teacher.noGroupSelected")
              }
              description={
                allGroups.length === 0
                  ? t("teacher.noGroupsHint")
                  : t("teacher.pickGroup")
              }
            />
          )}
      ...
      {isLessonModalOpen && (
        <StartLessonModal
          ...
          onSubmit={(topic) =>
            startLesson.mutate({ groupId: selectedGroupId, topic })
          }
